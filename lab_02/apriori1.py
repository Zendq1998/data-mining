...
def generateRules(L, supportData, minConf=0.7):
    ...
    Args:
        ...
        minConf 最小置信度
    Returns:
        bigRuleList 置信度规则列表（关于 (A->B+置信度) 3个字段的组合）
    """
    bigRuleList = []
...
def calcConf(freqSet, H, supportData, brl, minConf=0.7):
    """calcConf（对两个元素的频繁项，计算置信度，例如： {1,2}/{1} 或者 {1,2}/{2} 看是否满足条件）
    Args:
    ...
    """
    # 记录置信度大于最小置信度（minConf）的集合
    prunedH = []
    for conseq in H: # 假设 freqSet = frozenset([1, 3]), H = [frozenset([1]), frozenset([3])]，那么现在需要求出 frozenset([1]) -> frozenset([3]) 的置信度和 frozenset([3]) -> frozenset([1]) 的置信度
